...
void x86::write_string(int mem8_loc, string str)
{
    auto s = str.c_str();
    printf("%s\n", s);
...
uint8_t x86::ld8_phys(int mem8_loc)
{
    return phys_mem8[mem8_loc];
}
void x86::st8_phys(int mem8_loc, uint8_t x)
{
    phys_mem8[mem8_loc] = x;
}
int x86::ld32_phys(int mem8_loc)
{
    return phys_mem32[mem8_loc >> 2];
}
void x86::st32_phys(int mem8_loc, int x)
{
    uint32_t mem8_locu         = mem8_loc;
    phys_mem32[mem8_locu >> 2] = x;
}
void x86::tlb_set_page(int mem8_loc, int page_val, int set_write_tlb, int set_user_tlb)
{
...
void x86::tlb_flush_all1(int la)
{
    int n     = tlb_pages_count;
    int new_n = 0;
...
void x86Internal::cpu_dump(int OPbyte)
{
    ...
    if (filecheck_start <= count && count <= filecheck_end) {
...
int x86Internal::Instruction(int _N_cycles, ErrorInfo interrupt)
{
    ...
    do {
        ...
        int64_t  eip_tlb_val = _tlb_read_[tlbidx];
        int      OPbyte;
        ...
        if (((eip_tlb_val | eip_offset) & 0xfff) >= (4096 - 15 + 1)) {
            ...
            eip_tlb_val     = _tlb_read_[eip_offset2 >> 12];
            ...
            switch (OPbyte) {
                ...
                case 0x5f:
                    if (FS_usage_flag) {
                        mem8_loc           = regs[4];
                        uint32_t mem8_locu = mem8_loc;
                        last_tlb_val       = _tlb_read_[mem8_locu >> 12];
...
int x86Internal::__ld_8bits_mem8_read()
{
    ...
    uint32_t mem8_locu  = mem8_loc;
    int      idx        = mem8_locu >> 12;
    int      tlb_lookup = _tlb_read_[idx] ^ mem8_loc;
...
int x86Internal::ld_32bits_mem8_read()
{
    int      last_tlb_val;
    uint32_t mem8_locu = mem8_loc;
    return (((last_tlb_val = _tlb_read_[mem8_locu >> 12]) | mem8_loc) & 3 ? __ld_32bits_mem8_read()
                                                                          : phys_mem32[(mem8_loc ^ last_tlb_val) >> 2]);
}
...
int x86Internal::__ld_8bits_mem8_write()
{
    ...
    uint32_t mem8_locu  = mem8_loc;
    int      tlb_lookup = _tlb_write_[mem8_locu >> 12] ^ mem8_loc;
    return phys_mem8[tlb_lookup];
}
...
void x86Internal::st8_mem8_write(int x)
{
    uint32_t mem8_locu    = mem8_loc;
    int      idx          = mem8_locu >> 12;
    int      last_tlb_val = _tlb_write_[idx];
    if (last_tlb_val == -1) {
        __st8_mem8_write(x);
    } else {
        phys_mem8[mem8_loc ^ last_tlb_val] = x;
...
int x86Internal::do_tlb_lookup(int mem8_loc, int ud)
{
    ...
    uint32_t mem8_locu = mem8_loc;
    if (ud) {
        tlb_lookup = _tlb_write_[mem8_locu >> 12];
    } else {
        tlb_lookup = _tlb_read_[mem8_locu >> 12];
    }
    if (tlb_lookup == -1) {
...
{
    int CS_flags, l, mem8, local_OPbyte_var, base, conditional_var, stride;
    int n = 1;
...
int x86Internal::SS_mask_from_flags(int descriptor_high4bytes)
{
...
void x86Internal::load_from_descriptor_table(int selector, int *desary)
{
    ...
    int             Rb, descriptor_low4bytes, descriptor_high4bytes;
    ...
    desary[0] = descriptor_low4bytes;
    desary[1] = descriptor_high4bytes;
    return;
}
int x86Internal::calculate_descriptor_limit(int descriptor_low4bytes, int descriptor_high4bytes)
{
    int limit = (descriptor_low4bytes & 0xffff) | (descriptor_high4bytes & 0x000f0000);
    if (descriptor_high4bytes & (1 << 23))
        limit = (limit << 12) | 0xfff;
    return limit;
}
int x86Internal::calculate_descriptor_base(int descriptor_low4bytes, int descriptor_high4bytes)
{
    return (((descriptor_low4bytes >> 16) | ((descriptor_high4bytes & 0xff) << 16) |
...
void x86Internal::do_interrupt_protected_mode(int intno, int ne, int error_code, int oe, int pe)
{
    ...
    int             te, ue, is_32_bit;
    int             descriptor_low4bytes, descriptor_high4bytes, ve, ke, le, we, xe;
    int             ye, SS_mask;
    int             e[2];
    te = 0;
    ...
    set_segment_vars(1, selector, calculate_descriptor_base(descriptor_low4bytes, descriptor_high4bytes),
...
{
    DescriptorTable descriptor_table;
    int             descriptor_low4bytes, descriptor_high4bytes, Rb, De;
    selector &= 0xffff;
    if ((selector & 0xfffc) == 0) {
        ldt.base  = 0;
        ldt.limit = 0;
...
{
    DescriptorTable descriptor_table;
    int             descriptor_low4bytes, descriptor_high4bytes, Rb, descriptor_type, De;
    selector &= 0xffff;
    if ((selector & 0xfffc) == 0) {
        tr.base  = 0;
        tr.limit = 0;
...
{
    DescriptorTable descriptor_table;
    int             descriptor_low4bytes, descriptor_high4bytes, cpl_var, dpl, rpl, selector_index;
    cpl_var = cpl;
    if ((selector & 0xfffc) == 0) {    // null selector
        ...
        set_segment_vars(reg, selector, calculate_descriptor_base(descriptor_low4bytes, descriptor_high4bytes),
...
{
    int      Ne, ie, descriptor_low4bytes, descriptor_high4bytes, cpl_var, dpl, rpl;
    uint32_t limit;
    ...
    if (descriptor_high4bytes & (1 << 12)) {
        ...
        if ((Le >> 0) > (limit >> 0))
            abort_with_error_code(13, selector & 0xfffc);
        set_segment_vars(1, (selector & 0xfffc) | cpl_var,
                         calculate_descriptor_base(descriptor_low4bytes, descriptor_high4bytes), limit,
                         descriptor_high4bytes);
        eip = Le, physmem8_ptr = initial_mem_ptr = 0;
    } else {
...
{
    int dpl, descriptor_high4bytes;
    if ((reg == 4 || reg == 5) && (segs[reg].selector & 0xfffc) == 0)
        return;
    descriptor_high4bytes = segs[reg].flags;
    dpl                   = (descriptor_high4bytes >> 13) & 3;
    if (!(descriptor_high4bytes & (1 << 11)) || !(descriptor_high4bytes & (1 << 10))) {
        if (dpl < cpl_var) {
...
{
    int ue, i;
    int descriptor_low4bytes, descriptor_high4bytes, cpl_var, dpl, rpl, ve, Se;
...
void x86Internal::do_return_protected_mode(bool is_32_bit, bool is_iret, int imm16)
{
    ...
    int hf, jf, kf, lf;
    int descriptor_low4bytes, descriptor_high4bytes, we, xe;
...
{
    int descriptor_low4bytes, descriptor_high4bytes, rpl, dpl, cpl_var, descriptor_type;
    int e[2];
...
{
    int descriptor_low4bytes, descriptor_high4bytes, rpl, dpl, cpl_var;
    int e[2];
